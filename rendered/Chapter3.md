...
[3.2.3   Tree Recursion](#id9){.toc-backref}
--------------------------------------------
...
When computing the union of two sets, we must be careful not to include any element twice. The `union_set`{.docutils .literal} function also requires a linear number of membership tests, creating a process that also includes \\(\\Theta(n\^2)\\) steps.
...
**Handling exceptions.** An exception can be handled by an enclosing `try`{.docutils .literal} statement. A `try`{.docutils .literal} statement consists of multiple clauses; the first begins with `try`{.docutils .literal} and the rest begin with `except`{.docutils .literal}:
...
[3.5.1   Calculator](#id20){.toc-backref}
-----------------------------------------
...
>
>     When the resulting function is called, *ARGUMENTS* are bound to the argument values in a fresh environment frame that extends the environment in which the `lambda`{.docutils .literal} expression was evaluated (just like Python). Then the *BODY* is evaluated and its value returned as the value of the call.
>
...
> -   First, bindings are created for all the names defined by the `define`{.docutils .literal} statements, initially bound to undefined values.
...
>     > ```
>
>     The following function is not in [Revised(4) Scheme](http://people.csail.mit.edu/jaffer/r4rs_toc.html){.reference .external}, but is present in our versions of the interpreter (*warning:* a non-standard procedure that is not defined this way in later versions of Scheme):
>
>     ``` {.literal-block}
>     >>> (eval '(+ 1 2))
>     3
...
While `5`{.docutils .literal} and `five`{.docutils .literal} are interpreted differently, the Logo read-eval loop complains either way. The issue with the first case is that Logo complains whenever a top-level expression it evaluates does not evaluate to `None`{.docutils .literal}. Here, we see the first structural difference between the interpreters for Logo and Calculator; the interface to the former is a read-eval loop that expects the user to print results. The latter employed a more typical read-eval-print loop that printed return values automatically. Python takes a hybrid approach: only non-`None`{.docutils .literal} values are coerced to strings using `repr`{.docutils .literal} and then printed automatically.

A line of Logo can contain multiple expressions in sequence. The interpreter will evaluate each one in turn. It will complain if any top-level expression in a line does not evaluate to `None`{.docutils .literal}. Once an error occurs, the rest of the line is ignored:
...
Logo call expressions can be nested. In the version of Logo we will implement, each procedure takes a fixed number of arguments. Therefore, the Logo interpreter is able to determine uniquely when the operands of a nested call expression are complete. Consider, for instance, two procedures `sum`{.docutils .literal} and `difference`{.docutils .literal} that output the sum and difference of their two arguments, respectively:
...
Logo also supports infix operators, such as `+`{.docutils .literal} and `*`{.docutils .literal}. The precedence of these operators is resolved according to the standard rules of algebra; multiplication and division take precedence over addition and subtraction:
...
In addition to words, Logo includes the *sentence* type, interchangeably called a list. Sentences are enclosed in square brackets. The `print`{.docutils .literal} procedure does not show brackets to preserve the conversational style of Logo, but the square brackets can be printed in the output by using the `show`{.docutils .literal} procedure:
...
Sentences can be constructed using three different two-argument procedures. The `sentence`{.docutils .literal} procedure combines its arguments into a sentence. It is polymorphic; it places its arguments into a new sentence if they are words or concatenates its arguments if they are sentences. The result is always a sentence:
...
The `list`{.docutils .literal} procedure creates a sentence from two elements, which allows the user to create hierarchical data structures:
...
Finally, the `fput`{.docutils .literal} procedure creates a list from a first element and the rest of the list, as did the `Rlist`{.docutils .literal} Python constructor from earlier in the chapter:
...
Collectively, we can call `sentence`{.docutils .literal}, `list`{.docutils .literal}, and `fput`{.docutils .literal} the *sentence constructors* in Logo. Deconstructing a sentence into its `first`{.docutils .literal}, `last`{.docutils .literal}, and rest (called `butfirst`{.docutils .literal}) in Logo is straightforward as well. Hence, we also have a set of selector procedures for sentences:
...
The purpose of representing Logo expressions as sentences is typically not to print them out, but instead to evaluate them using the `run`{.docutils .literal} procedure:
...
Combining quotation, sentence constructors, and the `run`{.docutils .literal} procedure, we arrive at a very general means of combination that builds Logo expressions on the fly and then evaluates them:
...
At any moment during the course of executing a Logo program, the Logo turtle has a position and heading on the canvas. Single-argument procedures such as `forward`{.docutils .literal} and `right`{.docutils .literal} change the position and heading of the turtle. Common procedures have abbreviations: `forward`{.docutils .literal} can also be called as `fd`{.docutils .literal}, etc. The nested expression below draws a star with a smaller star at each vertex:
...
The full repertoire of Turtle procedures is also built into Python as the [turtle library module](http://docs.python.org/py3k/library/turtle.html){.reference .external}. A limited subset of these functions are exposed as Logo procedures in the companion project to this chapter.

**Assignment.** Logo supports binding names to values. As in Python, a Logo environment consists of a sequence of frames, and each frame can have at most one value bound to a given name. In Logo, names are bound with the `make`{.docutils .literal} procedure, which takes as arguments a name and a value:

``` {.literal-block}
? make "x 2
```

The first argument is the name `x`{.docutils .literal}, rather than the output of applying the procedure `x`{.docutils .literal}, and so it must be quoted. The values bound to names are retrieved by evaluating expressions that begin with a colon:
...
Any word that begins with a colon, such as `:x`{.docutils .literal}, is called a variable. A variable evaluates to the value to which the name of the variable is bound in the current environment.

The `make`{.docutils .literal} procedure does not have the same effect as an assignment statement in Python. The name passed to `make`{.docutils .literal} is either already bound to a value or is currently unbound.

1.  If the name is already bound, `make`{.docutils .literal} re-binds that name in the first frame in which it is found.
2.  If the name is not bound, `make`{.docutils .literal} binds the name in the global frame.

This behavior contrasts sharply with the semantics of the Python assignment statement, which always binds a name to a value in the first frame of the current environment. The first assignment rule above is similar to Python assignment following a `nonlocal`{.docutils .literal} statement. The second is similar to Python assignment following a `global`{.docutils .literal} statement.

**Procedures.** Logo supports user-defined procedures using definitions that begin with the `to`{.docutils .literal} keyword. Definitions are the final type of expression in Logo, along with call expressions, primitive expressions, and quoted expressions. The first line of a definition gives the name of the new procedure, followed by the formal parameters as variables. The lines that follow constitute the body of the procedure, which can span multiple lines and must end with a line that contains only the token `end`{.docutils .literal}. The Logo read-eval loop prompts the user for procedure bodies with a `>`{.docutils .literal} continuation symbol. Values are output from a user-defined procedure using the `output`{.docutils .literal} procedure:
...
A call to `output`{.docutils .literal} has the same role in Logo as a `return`{.docutils .literal} statement in Python: it halts the execution of the body of a procedure and returns a value. A Logo procedure can return no value at all by calling `stop`{.docutils .literal}:
...
While the name `x`{.docutils .literal} is not bound in the global frame, it is bound in the local frame for `print_x`{.docutils .literal}, the function that is called first. Logo's dynamic scoping rules allow the function `print_last_x`{.docutils .literal} to refer to `x`{.docutils .literal}, which was bound as the formal parameter of `print_x`{.docutils .literal}.

Dynamic scoping is implemented by a single change to the environment model of computation. The frame that is created by calling a user-defined function always extends the current environment. For example, the call to `print_x`{.docutils .literal} above introduces a new frame that extends the current environment, which consists solely of the global frame. Within the body of `print_x`{.docutils .literal}, the call to `print_last_x`{.docutils .literal} introduces another frame that extends the current environment, which includes both the local frame for `print_x`{.docutils .literal} and the global frame. As a result, looking up the name `x`{.docutils .literal} in the body of `print_last_x`{.docutils .literal} finds that name bound to `5`{.docutils .literal} in the local frame for `print_x`{.docutils .literal}. Alternatively, under the lexical scoping rules of Python, the frame for `print_last_x`{.docutils .literal} would have extended only the global frame and not the local frame for `print_x`{.docutils .literal}.

A dynamically scoped language has the advantage that its procedures may not need to take as many arguments. For instance, the `print_last_x`{.docutils .literal} procedure above takes no arguments, and yet its behavior can be parameterized by an enclosing scope.
...
There is no conditional expression type in Logo; the procedures `if`{.docutils .literal} and `ifelse`{.docutils .literal} are applied using call expression evaluation rules. The first argument of `if`{.docutils .literal} is a boolean word, either `True`{.docutils .literal} or `False`{.docutils .literal}. The second argument is not an output value, but instead a sentence that contains the line of Logo code to be evaluated if the first argument is `True`{.docutils .literal}. An important consequence of this design is that the contents of the second argument is not evaluated at all unless it will be used:
...
Not only does the Logo conditional expression not require a special syntax, but it can in fact be implemented in terms of `word`{.docutils .literal} and `run`{.docutils .literal}. The primitive procedure `ifelse`{.docutils .literal} takes three arguments: a boolean word, a sentence to be evaluated if that word is `True`{.docutils .literal}, and a sentence to be evaluated if that word is `False`{.docutils .literal}. By clever naming of the formal parameters, we can implement a user-defined procedure `ifelse2`{.docutils .literal} with the same behavior:
...
Recursive procedures do not require any special syntax, and they can be used with `run`{.docutils .literal}, `sentence`{.docutils .literal}, `first`{.docutils .literal}, and `butfirst`{.docutils .literal} to define general sequence operations on sentences. For instance, we can apply a procedure to an argument by building a two-element sentence and running it. The argument must be quoted if it is a word:
...
Next, we can define a procedure for mapping a procedure `:fn`{.docutils .literal} over the words in a sentence `:s`{.docutils .literal} incrementally:
...
The second line of the body of `map_fn`{.docutils .literal} can also be written with parentheses to indicate the nested structure of the call expression. However, parentheses show where call expressions begin and end, rather than surrounding only the operands and not the operator:
...
The `triangle`{.docutils .literal} procedure is a general method for repeating a drawing procedure three times with a left turn following each repetition. The `sierpinski`{.docutils .literal} procedure takes a length and a recursive depth. It draws a plain triangle if the depth is `1`{.docutils .literal}, and otherwise draws a triangle made up of calls to `leg`{.docutils .literal}. The `leg`{.docutils .literal} procedure draws a single leg of a recursive Sierpinski triangle by a recursive call to `sierpinski`{.docutils .literal} that fills the first half of the length of the leg, then by moving the turtle to the next vertex. The procedures `up`{.docutils .literal} and `down`{.docutils .literal} stop the turtle from drawing as it moves by lifting its pen up and the placing it down again. The mutual recursion between `sierpinski`{.docutils .literal} and `leg`{.docutils .literal} yields this result:
...
[3.6.3   Structure](#id25){.toc-backref}
----------------------------------------
...
A complete implementation of `parse_line`{.docutils .literal} appears in the companion projects as `logo_parser.py`{.docutils .literal}.

**Evaluation.** Logo is evaluated one line at a time. A skeleton implementation of the evaluator is defined in `logo.py`{.docutils .literal} of the companion project. The sentence returned from `parse_line`{.docutils .literal} is passed to the `eval_line`{.docutils .literal} function, which evaluates each expression in the line. The `eval_line`{.docutils .literal} function repeatedly calls `logo_eval`{.docutils .literal}, which evaluates the next full expression in the line until the line has been evaluated completely, then returns the last value. The `logo_eval`{.docutils .literal} function evaluates a single expression.

![](img/logo_eval.png)

The `logo_eval`{.docutils .literal} function evaluates the different forms of expressions that we introduced in the last section: primitives, variables, definitions, quoted expressions, and call expressions. The form of a multi-element expression in Logo can be determined by inspecting its first element. Each form of expression as its own evaluation rule.

1.  A primitive expression (a word that can be interpreted as a number, `True`{.docutils .literal}, or `False`{.docutils .literal}) evaluates to itself.
...
A simplified implementation of `logo_apply`{.docutils .literal} appears below. Some error checking has been removed in order to focus our discussion. A more robust implementation appears in the companion project.
...
The final case above invokes a second process, procedure application, that is expressed by a function `apply_procedure`{.docutils .literal}. To apply a procedure named by an operator token, that operator is looked up in the current environment. In the definition above, `env`{.docutils .literal} is an instance of the `Environment`{.docutils .literal} class described in the next section. The attribute `env.procedures`{.docutils .literal} is a dictionary that stores the mapping between operator names and procedures. In Logo, an environment has a single such mapping; there are no locally defined procedures. Moreover, Logo maintains separate mappings, called separate *namespaces*, for the the names of procedures and the names of variables. A procedure and an unrelated variable can have the same name in Logo. However, reusing names in this way is not recommended.

**Procedure application.** Procedure application begins by calling the `apply_procedure`{.docutils .literal} function, which is passed the procedure looked up by `logo_apply`{.docutils .literal}, along with the remainder of the current line of code and the current environment. The procedure application process in Logo is considerably more general than the `calc_apply`{.docutils .literal} function in Calculator. In particular, `apply_procedure`{.docutils .literal} must inspect the procedure it is meant to apply in order to determine its argument count *n*, before evaluating *n* operand expressions. It is here that we see why the Logo parser was unable to build an expression tree by syntactic analysis alone; the structure of the tree is determined by the procedure.

The `apply_procedure`{.docutils .literal} function calls a function `collect_args`{.docutils .literal} that must repeatedly call `logo_eval`{.docutils .literal} to evaluate the next *n* expressions on the line. Then, having computed the arguments to the procedure, `apply_procedure`{.docutils .literal} calls `logo_apply`{.docutils .literal}, the function that actually applies procedures to arguments. The call graph below illustrates the process.

![](img/logo_apply.png)

The final function `logo_apply`{.docutils .literal} applies two kinds of arguments: primitive procedures and user-defined procedures, both of which are instances of the `Procedure`{.docutils .literal} class. A `Procedure`{.docutils .literal} is a Python object that has instance attributes for the name, argument count, body, and formal parameters of a procedure. The type of the `body`{.docutils .literal} attribute varies. A primitive procedure is implemented in Python, and so its `body`{.docutils .literal} is a Python function. A user-defined (non-primitive) procedure is defined in Logo, and so its `body`{.docutils .literal} is a list of lines of Logo code. A `Procedure`{.docutils .literal} also has two boolean-valued attributes, one to indicated whether it is primitive and another to indicate whether it needs access to the current environment.
...
[3.6.4   Environments](#id26){.toc-backref}
-------------------------------------------

Now that we have described the structure of our Logo interpreter, we turn to implementing the `Environment`{.docutils .literal} class so that it correctly supports assignment, procedure definition, and variable lookup with dynamic scope. An `Environment`{.docutils .literal} instance represents the collective set of name bindings that are accessible at some point in the course of program execution. Bindings are organized into frames, and frames are implemented as Python dictionaries. Frames contain name bindings for variables, but not procedures; the bindings between operator names and `Procedure`{.docutils .literal} instances are stored separately in Logo. In the project implementation, frames that contain variable name bindings are stored as a list of dictionaries in the `_frames`{.docutils .literal} attribute of an `Environment`{.docutils .literal}, while procedure name bindings are stored in the dictionary-valued `procedures`{.docutils .literal} attribute.

Frames are not accessed directly, but instead through two `Environment`{.docutils .literal} methods: `lookup_variable`{.docutils .literal} and `set_variable_value`{.docutils .literal}. The first implements a process identical to the look-up procedure that we introduced in the environment model of computation in Chapter 1. A name is matched against the bindings of the first (most recently added) frame of the current environment. If it is found, the value to which it is bound is returned. If it is not found, look-up proceeds to the frame that was extended by the current frame.

The `set_variable_value`{.docutils .literal} method also searches for a binding that matches a variable name. If one is found, it is updated with a new value. If none is found, then a new binding is created in the global frame. The implementations of these methods are left as an exercise in the companion project.

The `lookup_variable`{.docutils .literal} method is invoked from `logo_eval`{.docutils .literal} when evaluating a variable name. The `set_variable_value`{.docutils .literal} method is invoked by the `logo_make`{.docutils .literal} function, which serves as the body of the primitive `make`{.docutils .literal} procedure in Logo.
...
With the addition of variables and the `make`{.docutils .literal} primitive, our interpreter supports its first means of abstraction: binding names to values. In Logo, we can now replicate our first abstraction steps in Python from Chapter 1:
...
Assignment is only a limited form of abstraction. We have seen from the beginning of this course that user-defined functions are a critical tool in managing the complexity of even moderately sized programs. Two enhancements will enable user-defined procedures in Logo. First, we must describe the implementation of `eval_definition`{.docutils .literal}, the Python function called from `logo_eval`{.docutils .literal} when the current line is a definition. Second, we will complete our description of the process in `logo_apply`{.docutils .literal} that applies a user-defined procedure to some arguments. Both of these changes leverage the `Procedure`{.docutils .literal} class defined in the previous section.

A definition is evaluated by creating a new `Procedure`{.docutils .literal} instance that represents the user-defined procedure. Consider the following Logo procedure definition:
...
The first line of the definition supplies the name `factorial`{.docutils .literal} and formal parameter `n`{.docutils .literal} of the procedure. The line that follows constitute the body of the procedure. This line is not evaluated immediately, but instead stored for future application. That is, the line is read and parsed by `eval_definition`{.docutils .literal}, but not passed to `eval_line`{.docutils .literal}. Lines of the body are read from the user until a line containing only `end`{.docutils .literal} is encountered. In Logo, `end`{.docutils .literal} is not a procedure to be evaluated, nor is it part of the procedure body; it is a syntactic marker of the end of a procedure definition.

The `Procedure`{.docutils .literal} instance created from this procedure name, formal parameter list, and body, is registered in the `procedures`{.docutils .literal} dictionary attribute of the environment. In Logo, unlike Python, once a procedure is bound to a name, no other definition can use that name again.

The `logo_apply`{.docutils .literal} function applies a `Procedure`{.docutils .literal} instance to some arguments, which are Logo values represented as strings (for words) and lists (for sentences). For a user-defined procedure, `logo_apply`{.docutils .literal} creates a new frame, a dictionary object in which the the keys are the formal parameters of the procedure and the values are the arguments. In a dynamically scoped language such as Logo, this new frame always extends the current environment in which the procedure was called. Therefore, we append the newly created frame onto the current environment. Then, each line of the body is passed to `eval_line`{.docutils .literal} in turn. Finally, we can remove the newly created frame from the environment after evaluating its body. Because Logo does not support higher-order or first-class procedures, we never need to track more than one environment at a time throughout the course of execution of a program.
...
[3.6.5   Data as Programs](#id27){.toc-backref}
-----------------------------------------------
